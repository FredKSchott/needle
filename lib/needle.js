...
// const {version} = JSON.stringify(fs.readFileSync(__filename + '../package.json', 'utf-8'));
const version = '1.6.0';
...
export default function needle() {
  ...
  return new Promise(function(resolve, reject) {
    needle.request(verb, args[0], args[1], args[2], function(err, resp) {
...
'head get'.split(' ').forEach(function(method) {
  needle[method] = function(uri, options, callback) {
    return new Needle(method, uri, null, options, callback).start();
  }
})

'post put patch delete'.split(' ').forEach(function(method) {
  needle[method] = function(uri, data, options, callback) {
    return new Needle(method, uri, data, options, callback).start();
  }
})

needle.request = function(method, uri, data, opts, callback) {
